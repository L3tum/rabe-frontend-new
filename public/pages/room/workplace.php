...
use RaBe\Models\Category;
use RaBe\Util\Serializer;
use RaBe\Models\Error;
...
/** @var Error[] $errors */
$errors = Serializer::deserializeArray($response, Error::class);
    ...
    <div class="row mt-lg-5 mt-2">
        ...
        <?php if ($errors !== null && count($errors) > 0): ?>
            <?php foreach ($errors as $error): ?>
                <?php
                $response = makeGetRequest($backend . '/api/categories/' . $error->kategorieId, $user->getToken(), 200);

                /** @var Category $category */
                $category = Serializer::deserialize($response, Category::class);
                ?>
                    ...
                    <div class="card" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            ...
                            <button type="button" class="btn btn-dark btn-block"
                                    onclick="resolveError(<?php echo $error->id; ?>)">Beheben
                            </button>
                        </div>
                    </div>
                </div>
